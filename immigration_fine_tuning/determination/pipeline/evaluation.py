...
def load_data(base_path):
    ...
    train_path = Path(base_path) / "train_enriched.csv"
    test_path = Path(base_path) / "test_enriched.csv"
...
def run_evaluation(model, validation_file=None, output_dir=None, analyze_failures=True, analyze_overextraction=True):
    ...
    if validation_file is None:
        validation_file = Path(__file__).parent / "validation_set.csv"
    ...
    if output_dir is None:
        # Use the directory containing the validation file as default
...
def main():
    # Use relative paths instead of hard-coded local directories
    base_path = Path(__file__).parent.parent.parent / "data" / "merged"
    
    # Uncomment these lines to recreate the validation set
    ...
    # output_dir.mkdir(exist_ok=True)
    # validation_set.to_csv(output_dir / "validation_set.csv", index=False)
