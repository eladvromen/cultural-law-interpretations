...
class SectionBasedBasicExtractor:
    ...
    def process_dataframe(self, df):
        ...
        for section in sections:
            ...
            df[count_column] = df[output_column].apply(
                lambda x: len(x) if isinstance(x, list) else 0
            )
            df[raw_count_column] = df[output_column].apply(
                lambda x: len(x) if isinstance(x, list) else 0
            )
        
        # Add count for sparse extraction if it exists
        if 'sparse_explicit_extraction' in df.columns:
            df['sparse_explicit_extraction_count'] = df['sparse_explicit_extraction'].apply(
                lambda x: len(x) if isinstance(x, list) else 0
            )
...
def main():
    ...
    # Define base paths
    base_dir = Path('C:/Users/shil6369/cultural-law-interpretations/immigration_fine_tuning')
    rule_based_dir = base_dir / "determination" / "pipeline" / "rule_based"
    
    # Create pipeline_stages directory if it doesn't exist
    pipeline_stages_dir = rule_based_dir / "pipeline_stages"
    pipeline_stages_dir.mkdir(exist_ok=True)
    ...
    if 'sparse_explicit_extraction_count' in results_df.columns:
        logger.info(f"  Sparse explicit extraction: {results_df['sparse_explicit_extraction_count'].sum()} total, " 
                   f"{results_df['sparse_explicit_extraction_count'].mean():.2f} avg per document")
    ...
    for section in ['decision', 'analysis', 'reasons', 'conclusion']:
        ...
        if count_col in results_df.columns:
            logger.info(f"  {section.capitalize()} basic extraction: {results_df[count_col].sum()} total, "
                       f"{results_df[count_col].mean():.2f} avg per document")
